...
def main():
    ...
    * decode first the evidence, then run classification for each split
    ...
    unk_token = 'UNK'
    evidence_identifier, evidence_classifier, word_interner, de_interner, evidence_classes = initialize_models(model_params, vocab, batch_first=BATCH_FIRST, unk_token=unk_token)
    ...
    evidence_identifier, evidence_ident_results = train_evidence_identifier(evidence_identifier.cuda(), args.output_dir, interned_train, interned_val, interned_documents, model_params, tensorize_model_inputs=True)
    evidence_classifier, evidence_class_results = train_evidence_classifier(evidence_classifier.cuda(), args.output_dir, interned_train, interned_val, interned_documents, model_params, class_interner=evidence_classes, tensorize_model_inputs=True)
    pipeline_batch_size = min([model_params['evidence_classifier']['batch_size'], model_params['evidence_identifier']['batch_size']])
    pipeline_results, train_decoded, val_decoded, test_decoded = decode(evidence_identifier, evidence_classifier, interned_train, interned_val, interned_test, interned_documents, evidence_classes, pipeline_batch_size, tensorize_model_inputs=True)
